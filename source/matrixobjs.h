...
inline size_t toZeroIndex(size_t oneIndex) {
	return oneIndex - 1;
}
inline size_t toOneIndex(size_t zeroIndex) {
	return zeroIndex + 1;
}
...
public:
	...
	DimVector getMaxIndices(const ArrayObj* pSlice,
			const BaseMatrixObj* pAssignMatrix = NULL) const;
	...
	static void expandMatrix(BaseMatrixObj* pMatrix, size_t* pIndices,
			size_t numIndices);
	...
	virtual void setSliceND(const ArrayObj* pSlice,
			const DataObject* pSubMatrix) = 0;

	// Method to concatenate this matrix with another matrix
	virtual BaseMatrixObj* concat(const BaseMatrixObj* pOther,
			size_t catDim) const = 0;

	// Static method to test if matrices are compatible for multiplication
	static bool multCompatible(const BaseMatrixObj* pMatrixA,
			const BaseMatrixObj* pMatrixB);

	// Static method to test if matrices are compatible for left division
	static bool leftDivCompatible(const BaseMatrixObj* pMatrixA,
			const BaseMatrixObj* pMatrixB);

	// Method to test if the matrix is a scalar value
	virtual bool isScalar() const {
		return (m_size.size() == 2 && m_size[0] == 1 && m_size[1] == 1);
	}
	...
	bool isSquare() const {
		return m_size.size() == 2 && m_size[0] == m_size[1];
	}
	...
	static size_t getDimCount(const BaseMatrixObj* pMatrix) {
		return pMatrix->m_size.size();
	}

	// Static method to get a pointer to the size array
	static const size_t* getSizeArray(const BaseMatrixObj* pMatrix) {
		return &pMatrix->m_size[0];
	}
	virtual const DimVector& getDataSize() const{
		#ifdef DEBUG
		std::cout<<"GetDataSize method invoked"<<std::endl;
		#endif
		return getSize();
	}
	// implement in matrixObj class
	...
	virtual void setDimensions(long* dim, long ndims) {
		m_size.resize(ndims);
...
protected:
	...
	void getSliceND(const ArrayObj* pSlice, size_t curDim, size_t* pIndices,
			ScalarType*& pDstElem) const;
...
public:
	...
	MatrixObj(const DimVector& newSize, ScalarType elemValue = 0) {
		...
		for (DimVector::const_iterator itr = m_size.begin();
				itr != m_size.end(); ++itr)
			elemCount *= *itr;
	...
	virtual MatrixObj* copy() const {
		...
		memcpy(pNewMatrix->m_pElements, m_pElements,
				sizeof(ScalarType) * m_numElements);
	...
	virtual void expand(const DimVector& indices) {
		...
		expand(oldSize, newSize, srcStride, dstStride, pOldElements,
				m_pElements, newSize.size() - 1);
	...
	void expand(const DimVector& srcSize, const DimVector& dstSize,
			const DimVector& srcStride, const DimVector& dstStride,
			const ScalarType* pSrc, ScalarType* pDst, size_t curDim) {
		...
		if (curDim == 0) {
			...
			std::uninitialized_fill(pDst + srcSize[0], pDst + dstSize[0],
					(ScalarType) 0);
		} else {
			...
			for (size_t i = 0; i < srcSize[curDim]; ++i) {
				...
				expand(srcSize, dstSize, srcStride, dstStride, pSrcSlice,
						pDstSlice, curDim - 1);
	...
	virtual MatrixObj* getSliceND(const ArrayObj* pSlice) const {
		...
		for (size_t i = 0; i < pSlice->getSize(); ++i) {
			...
			if (pCurSlice->isMatrixObj()) {
				...
				if (pCurSlice->getType() == DataObject::LOGICALARRAY) {
					...
					for (const bool* pValue = pStartVal; pValue != pEndVal;
							++pValue) {
		...
		if (isVector()) {
			...
			if ((m_size[0] != 1 && newSize[1] != 1)
					|| (m_size[1] != 1 && newSize[0] != 1)) {
	...
	void getSliceND(const ArrayObj* pSlice, size_t curDim, size_t* pIndices,
		...
		if (curDim > 0) {
			...
			if (pCurSlice->getType() == DataObject::MATRIX_F64) {
				...
				// For each value in the matrix
				for (const float64* pValue = pStartVal; pValue != pEndVal;
						++pValue) {
			...
			// If the object is a range
				...
				if (pRange->isFullRange()) {
					...
					if (curDim == pSlice->getSize() - 1) {
						// Compute the size of the extended last dimension
						for (size_t dim = curDim + 1; dim < m_size.size();
								++dim)
	...
	// Helper method to recursively implement slice copying
	void setSliceND(const ArrayObj* pSlice, size_t curDim, size_t* pIndices,
			ScalarType*& pSrcElem) const {
		// Get a reference to the slice along the current dimension
		const DataObject* pCurSlice = pSlice->getObject(curDim);
		...
		if (m_type != pOther->getType()) {
			...
			if (pOther->getType() == MATRIX_C128) {
				...
				return MatrixObj<Complex128>::concat(
						...
						(MatrixObj<Complex128>*) pOther, catDim);
			}
	...
	ScalarType getElem2D(size_t rowIndex, size_t colIndex) const {
		...
		assert(
				m_numElements > 0 && rowIndex < m_size[0]
						&& colIndex < m_size[1]);
	...
	static MatrixObj* conjTranspose(const MatrixObj* pMatrix) {
		...
		MatrixObj* pResult = new MatrixObj(pMatrix->m_size[1],
				pMatrix->m_size[0]);
		...
		for (size_t i = 0; i < numRows; ++i) {
			...
			for (size_t j = 0; j < numCols; ++j) {
				...
				pResult->m_pElements[i * numCols + j] = std::conj(
						pMatrix->m_pElements[j * numRows + i]);
			}
		}
	...
	static MatrixObj* transpose(const MatrixObj* pMatrix) {
		...
		for (size_t i = 0; i < numRows; ++i) {
			...
			for (size_t j = 0; j < numCols; ++j) {
				...
				pResult->m_pElements[i * numCols + j] = pMatrix->m_pElements[j
						* numRows + i];
			}
		}
	...
	static MatrixObj* matrixMult(const MatrixObj* pMatrixA,
			const MatrixObj* pMatrixB) {
	...
	template<class InScalarType> static MatrixObj* scalarMult(
			const MatrixObj* pMatrix, InScalarType scalar) {
		// Perform the scalar multiplication
		return scalarMult(pMatrix, (ScalarType) scalar);
	}

	// Static method to perform matrix left division
	static MatrixObj* matrixLeftDiv(const MatrixObj* pMatrixA,
			const MatrixObj* pMatrixB) {
		// Default version unimplemented, see specialized versions
		assert(false);
	}
	...
	static MatrixObj* matrixRightDiv(const MatrixObj* pMatrixA,
			const MatrixObj* pMatrixB) {
		// Default version unimplemented, see specialized versions
		assert(false);
	}
	...
	template<class UnaryOp, class OutType> static MatrixObj<OutType>* arrayOp(
			const MatrixObj* pMatrix) {
		// Create a new matrix object to store the result
		MatrixObj<OutType>* pResult = new MatrixObj<OutType>(pMatrix->m_size);

		// Compute a pointer to the last matrix element of the matrix
		const ScalarType* pLastElem = pMatrix->m_pElements
	...
	template<class BinaryOp, class OutType, class InScalarType> static MatrixObj<
			OutType>* lhsScalarArrayOp(const MatrixObj* pMatrixR,
			InScalarType scalarL) {
		// Create a new matrix object to store the result
		...
		const ScalarType* pLastElem = pMatrixR->m_pElements
				+ pMatrixR->m_numElements;
	...
	template<class BinaryOp, class OutType, class InScalarType> static MatrixObj<
			OutType>* rhsScalarArrayOp(const MatrixObj* pMatrixL,
			InScalarType scalarR) {
		// Create a new matrix object to store the result
		MatrixObj<OutType>* pResult = new MatrixObj<OutType>(pMatrixL->m_size);

		// Compute a pointer to the last matrix element of the right matrix
		const ScalarType* pLastElem = pMatrixL->m_pElements
				+ pMatrixL->m_numElements;
	...
	template<class BinaryOp, class OutType> static MatrixObj<OutType>* binArrayOp(
			const MatrixObj* pMatrixA, const MatrixObj* pMatrixB) {
		// If the left matrix is a scalar
		if (pMatrixA->isScalar()) {
			// Perform the operation with a scalar lhs and a matrix rhs
			return lhsScalarArrayOp<BinaryOp, OutType, ScalarType>(pMatrixB,
					pMatrixA->getScalar());
		}
		...
		// If the right matrix is a scalar
			...
			return rhsScalarArrayOp<BinaryOp, OutType, ScalarType>(pMatrixA,
					pMatrixB->getScalar());
		}
		...
		// Otherwise, if both matrices are non-scalar
			...
			// Create a new matrix object to store the result
			MatrixObj<OutType>* pResult = new MatrixObj<OutType>(
					pMatrixA->m_size);

			// Compute a pointer to the last matrix element
			const ScalarType* pLastElem = pMatrixA->m_pElements
					+ pMatrixA->m_numElements;
	...
	template<class VectorOp, class OutType> static MatrixObj<OutType>* vectorOp(
			const MatrixObj* pInMatrix, size_t opDim) {
		...
		for (;;) {
			...
			pVecEnd = std::min(pVecEnd,
					pInMatrix->m_pElements + pInMatrix->m_numElements);

			// Ensure that the output pointer is valid
			assert(
					pOut
							< pOutMatrix->getElements()
									+ pOutMatrix->getNumElems());
	...
	static MatrixObj* concat(const MatrixObj* pMatrixA,
			const MatrixObj* pMatrixB, size_t catDim) {
		...
		for (size_t i = 0; i < numSlices; ++i) {
			...
			if (sliceSizeA > 0) {
				...
				const ScalarType* pSliceA = pMatrixA->m_pElements
						+ i * sliceSizeA;
			...
			if (sliceSizeB > 0) {
				...
				const ScalarType* pSliceB = pMatrixB->m_pElements
						+ i * sliceSizeB;

				// Copy the data into the result matrix
				memcpy(pSliceR + sliceSizeA, pSliceB,
						sizeof(ScalarType) * sliceSizeB);
			}
		}
	...
	static ScalarType readElem2D(const MatrixObj* pMatrix, int64 index1,
			int64 index2) {
		...
		if (zeroIndex1 >= pMatrix->m_size[0]
				|| offset >= pMatrix->m_numElements)
	...
	static void writeElem2D(MatrixObj* pMatrix, int64 index1, int64 index2,
			ScalarType value) {
		// Convert the indices to zero indexing
		size_t zeroIndex1 = toZeroIndex(size_t(index1));
		size_t zeroIndex2 = toZeroIndex(size_t(index2));
	...
	const ScalarType* getElements() const {
		return m_pElements;
	}

	// Accessors to access the matrix elements
	ScalarType* getElements() {
		return m_pElements;
	}

	// Static method to get the first element of a matrix
	static ScalarType getScalarVal(const MatrixObj* pMatrix) {
		return pMatrix->getScalar();
	}
	...
	typedef MatrixObj<bool>* (*MATRIX_LOGIC_OP_FUNC)(const MatrixObj*,
			const MatrixObj*);
	typedef MatrixObj<bool>* (*SCALAR_LOGIC_OP_FUNC)(const MatrixObj*,
			ScalarType);
	typedef MatrixObj<bool>* (*F64_SCALAR_LOGIC_OP_FUNC)(const MatrixObj*,
			float64);
	...
	virtual double getScalarDouble() const {
		...
		return static_cast<double>(getScalar());

	}
	virtual void* getData() const {
		return static_cast<void*>(m_pElements);
	}
	virtual void setData(void* data) {
		m_pElements = static_cast<ScalarType*>(data);
	}
	virtual void* getImagData() const {
		return NULL;
	}
	virtual void setImagData(void* data) {

	}
	virtual DataObject* getCell(long idx)const  {
	#ifdef DEBUG
	std::cout<<"Not A Cell Array. Returning NULL"<<std::endl;
	#endif	
		return NULL;	
	}
	virtual void setCell(long idx,DataObject*obj){
	#ifdef DEBUG
	std::cout<<"Not A Cell Array. Doing Nothing"<<std::endl;
	#endif	
	}
...
protected:
	...
	void allocMatrix() {
		...
		#ifdef MCVM_USE_GC
		m_pElements =
				(ScalarType*) GC_MALLOC_ATOMIC_IGNORE_OFF_PAGE(m_numElements * sizeof(ScalarType));
		#else
	...
	void initMatrix(ScalarType value) {
		...
		std::uninitialized_fill(m_pElements, m_pElements + m_numElements,
				value);
	}
...
template<> inline DataObject::Type MatrixObj<Complex64>::getClassType() {
	return DataObject::MATRIX_C64;
}
template<> inline DataObject::Type MatrixObj<float32>::getClassType() {
	return DataObject::MATRIX_F32;
}
template<> inline DataObject::Type MatrixObj<float64>::getClassType() {
	return DataObject::MATRIX_F64;
}
template<> inline DataObject::Type MatrixObj<Complex128>::getClassType() {
	return DataObject::MATRIX_C128;
}
template<> inline DataObject::Type MatrixObj<bool>::getClassType() {
	return DataObject::LOGICALARRAY;
}
template<> inline DataObject::Type MatrixObj<char>::getClassType() {
	return DataObject::CHARARRAY;
}
template<> inline DataObject::Type MatrixObj<DataObject*>::getClassType() {
	return DataObject::CELLARRAY;
}
template<> inline DataObject::Type MatrixObj<int32>::getClassType(){
	return DataObject::MATRIX_I32;
}
template<> inline DataObject::Type MatrixObj<int16>::getClassType(){
	return DataObject::MATRIX_I16;
}
template<> inline DataObject::Type MatrixObj<int64>::getClassType(){
	return DataObject::MATRIX_I64;
}
template<> inline DataObject::Type MatrixObj<int8>::getClassType(){
	return DataObject::MATRIX_I8;
}
template<> inline DataObject::Type MatrixObj<uint32>::getClassType(){
	return DataObject::MATRIX_UI32;
}
template<> inline DataObject::Type MatrixObj<uint16>::getClassType(){
	return DataObject::MATRIX_UI16;
}
template<> inline DataObject::Type MatrixObj<uint64>::getClassType(){
	return DataObject::MATRIX_UI64;
}
template<> inline DataObject::Type MatrixObj<uint8>::getClassType(){
	return DataObject::MATRIX_UI8;
}
// Template specialization of the conversion method
template<>  DataObject* MatrixObj<Complex128>::convert(
		DataObject::Type outType) const;
template<> DataObject* MatrixObj<DataObject*>::convert(
		DataObject::Type outType) const;

template<>  DataObject* MatrixObj<Complex64>::convert(
		DataObject::Type outType) const;
// Template specialization of the matrix multiplication method
template<> MatrixObj<float64>* MatrixObj<float64>::matrixMult(
		const MatrixObj* pMatrixA, const MatrixObj* pMatrixB);
template<> MatrixObj<Complex128>* MatrixObj<Complex128>::matrixMult(
		const MatrixObj* pMatrixA, const MatrixObj* pMatrixB);
template<> MatrixObj<Complex64>* MatrixObj<Complex64>::matrixMult(
		const MatrixObj* pMatrixA, const MatrixObj* pMatrixB);
// Template specialization of the scalar multiplication method
template<> MatrixObj<float64>* MatrixObj<float64>::scalarMult(
		const MatrixObj* pMatrix, float64 scalar);
template<> MatrixObj<Complex128>* MatrixObj<Complex128>::scalarMult(
		const MatrixObj* pMatrix, Complex128 scalar);
template<> MatrixObj<Complex64>* MatrixObj<Complex64>::scalarMult(
		const MatrixObj* pMatrix, Complex64 scalar);
// Template specialization of the matrix left-division method
template<> MatrixObj<float64>* MatrixObj<float64>::matrixLeftDiv(
		const MatrixObj* pMatrixA, const MatrixObj* pMatrixB);
template<> MatrixObj<Complex128>* MatrixObj<Complex128>::matrixLeftDiv(
		const MatrixObj* pMatrixA, const MatrixObj* pMatrixB);
template<> MatrixObj<Complex64>* MatrixObj<Complex64>::matrixLeftDiv(
		const MatrixObj* pMatrixA, const MatrixObj* pMatrixB);

// Template specialization of the matrix right-division method
template<> MatrixObj<float64>* MatrixObj<float64>::matrixRightDiv(
		const MatrixObj* pMatrixA, const MatrixObj* pMatrixB);
template<> MatrixObj<Complex128>* MatrixObj<Complex128>::matrixRightDiv(
		const MatrixObj* pMatrixA, const MatrixObj* pMatrixB);
template<> MatrixObj<Complex64>* MatrixObj<Complex64>::matrixRightDiv(
		const MatrixObj* pMatrixA, const MatrixObj* pMatrixB);

// Template specialization of the range initialization method
template<> void MatrixObj<DataObject*>::initRange(DataObject** pStart,
		DataObject** pEnd);
...
typedef MatrixObj<Complex64> MatrixC64Obj;
